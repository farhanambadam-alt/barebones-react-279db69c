...
export function useFlutterBridge() {
  ...
  useEffect(() => {
    ...
    callFlutterHandler('routeChanged', path);
  }, [location.pathname]);
  ...
  useEffect(() => {
    ...
    window.appBack = () => {
      ...
      if (overlayCloseStack.length > 0) {
        ...
        callFlutterHandler('routeChanged', currentPath);
        return;
      }
      ...
      if (TAB_ROUTES.has(currentPath) && currentPath !== '/') {
        ...
        callFlutterHandler('routeChanged', '/');
        return;
      }
      ...
      callFlutterHandler('routeChanged', previous);
    };
