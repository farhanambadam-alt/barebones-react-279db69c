...
import { ArrowLeft, Star, MapPin, Clock, Plus, Minus, Navigation, Heart, ChevronDown, Share2, Scissors, Sparkles, Award, Instagram, X, Play, Copy, Gift, Tag, Info, MessageSquareText, Images } from 'lucide-react';
...
import { useSalonById, useServices, useArtists, useReviews } from '@/services/hooks';
import SalonTag from '@/components/SalonTag';
import JourneyCard from '@/components/salon/JourneyCard';
...
const SalonDetail = () => {
  ...
  const dynamicDistance = salon ? getDistance(salon) : '';
  ...
  /* ── Group individual services by subcategory (packages stay in drawer) ── */
  // Removed emoji icons — using primary accent bar instead

  const individualServices = filteredServices.filter((s) => s.category !== 'packages');
  const groupedServices = individualServices.reduce<Record<string, typeof individualServices>>((acc, service) => {
  ...
  const getPromoPillStyle = (tab: 'packages' | 'offers') => {
    const isActive = tab === 'packages' ? packagesDrawerOpen : offersDrawerOpen;
  ...
  return (
    ...
    >
        ...
        <div className="mt-5 flex items-start justify-between gap-6">
          <div className="flex-1">
            ...
            <div className="flex gap-1.5 mt-3 flex-wrap">
              {salon.tags.map((tag) => (
                <SalonTag key={tag} label={tag} verified={tag === 'Verified'} />
              ))}
              {dynamicDistance && (
                <SalonTag label={dynamicDistance} distance />
              )}
            </div>
          </div>
          <div className="flex gap-2 flex-shrink-0 items-center">
            ...
            >
              <Gift size={14} className="flex-shrink-0 text-success" />
            ...
            >
              <Tag size={14} className="flex-shrink-0 text-accent" />
            ...
            {Object.entries(groupedServices).map(([subcategory, subServices]) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 px-4 pt-3.5 pb-2">
                  ...
                  <h3 className="font-heading font-semibold text-[15px] text-foreground tracking-tight">
                    {subcategory}
                  </h3>
                </div>

                {/* Service items */}
                <div className="px-2.5 space-y-2 pb-3 md:grid md:grid-cols-2 md:gap-2 md:space-y-0">
                  {subServices.map((service) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-heading font-medium text-[14px] text-foreground leading-snug">{service.name}</h4>
                        <span className="text-[12px] font-body text-muted-foreground/80 bg-secondary px-2 py-0.5 rounded-md inline-block mt-1.5">
                          {service.duration}
                        </span>
                        <div className="flex items-center gap-2 mt-1.5">
                          <span className="font-heading font-bold text-[16px] text-foreground">₹{service.price}</span>
                          {service.originalPrice && (
                            <span className="text-[11px] text-muted-foreground/60 line-through font-body">₹{service.originalPrice}</span>
                          )}
                        </div>
                      </div>

                      {/* Add / Qty — fixed width to prevent title reflow */}
                      <div className="w-[88px] flex-shrink-0 flex justify-end">
                        {cart[service.id] ? (
                          <div className="flex items-center gap-1 bg-primary/8 rounded-xl px-1 border border-primary/15">
                          ...
                          <button
                            ...
                            className="btn-themed text-[12px] font-heading font-semibold px-4 py-2.5 rounded-xl min-h-[40px]"
                          >
                            Add
                          </button>
                        )}
      ...
      {/* Packages Drawer */}
      <PromoDrawer open={packagesDrawerOpen} onClose={() => setPackagesDrawerOpen(false)}>
        <div className="px-5 pt-2 pb-6">
          <h2 className="font-heading font-semibold text-[17px] text-foreground mb-4 flex items-center gap-2">
      ...
      {/* Offers Drawer */}
      <PromoDrawer open={offersDrawerOpen} onClose={() => setOffersDrawerOpen(false)}>
        <div className="px-5 pt-2 pb-6">
          <h2 className="font-heading font-semibold text-[17px] text-foreground mb-4 flex items-center gap-2">
