...
import { useSalonById, useServices, useArtists } from '@/services/hooks';
...
import { initiatePayment, type PaymentRequest } from '@/services/api/payments';
import { rescheduleBooking, toDateOnly } from '@/services/api/bookings';
...
const BookingFlow = () => {
  ...
  const { salon } = useSalonById(id);
  const { services } = useServices(id);
  const { artists } = useArtists(id);

  // Reschedule mode: launched from Bookings.tsx with `state.reschedule = true`.
  // In this mode we skip the summary/payment step entirely — only date/time
  // and barber selection, then a direct confirm against the bookings API.
  const isReschedule: boolean = !!location.state?.reschedule;
  const rescheduleBookingId: string | undefined = location.state?.bookingId;
  ...
  const { draft, update: updateDraft, clear: clearDraft } = useBookingDraft(id);

  // In reschedule mode, never land on the summary step (no payment).
  const initialStep: Step = (() => {
    const s = draft?.step ?? 'datetime';
    if (isReschedule && s === 'summary') return 'barber';
    return s;
  })();
  ...
  const [selectedTime, setSelectedTime] = useState<string | null>(draft?.selectedTime ?? null);
  const [selectedBarber, setSelectedBarber] = useState<string | null>(draft?.selectedBarber ?? null);
  const [autoAssign, setAutoAssign] = useState(draft?.autoAssign ?? false);
  const [bookingSuccess, setBookingSuccess] = useState(false);
  const [paying, setPaying] = useState(false);

  // Sync draft on every meaningful change
  useEffect(() => {
    if (!id || bookingSuccess) return;
    updateDraft({
      step,
  ...
  }, [step, selectedDate, selectedTime, selectedBarber, autoAssign, id]);
  ...
  // In reschedule mode the "barber step" CTA confirms directly (no summary).
  const canConfirmReschedule = canProceedToBarber && canProceedToSummary;

  const { clearCart } = useCart();
  const { user, isGuest, requestAuth } = useCustomerAuth();

  const handleConfirmBooking = async () => {
    if (!salon) return;
    if (isGuest) {
      requestAuth(isReschedule ? 'Sign in to reschedule' : 'Sign in to confirm your booking');
      return;
    }
    if (paying) return;
    setPaying(true);
    try {
      // ─── Reschedule path ────────────────────────────────────────────
      // Skips summary/payment. Hits the bookings API directly.
      // Backend swap: replace `rescheduleBooking` body with the real
      // Supabase update — payload shape is already final.
      if (isReschedule) {
        if (!rescheduleBookingId) {
          toast.error('Missing booking reference. Please try again.');
          return;
        }
        if (!selectedTime) {
        ...
        const result = await rescheduleBooking({
          ...
          date: toDateOnly(selectedDate), // 'YYYY-MM-DD' — matches DB DATE column
          time: selectedTime,
          artistId: autoAssign ? null : selectedBarber,
        });
        if (result.ok) {
          ...
          toast.error(result.error ?? 'Reschedule failed. Please try again.');
        }
        return;
      }

      // ─── New booking path (payment) ────────────────────────────────
      const totalWithTax = cartTotal + Math.round(cartTotal * 0.18);
      ...
      toast.error(e instanceof Error ? e.message : (isReschedule ? 'Reschedule failed' : 'Booking failed'));
    } finally {
      setPaying(false);
    }
  };
  ...
  if (!salon) {
    return (
      <ErrorState
        ...
        message="We couldn't load the salon for this booking."
        onRetry={() => navigate('/')}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <header className="sticky top-0 z-40 bg-card/90 backdrop-blur-md border-b border-border/50 px-4 pb-3" style={{ paddingTop: 'calc(var(--inset-top) + 12px)' }}>
        ...
        {/* Step indicator — reschedule mode hides the summary/payment step */}
        {(() => {
          const steps: Step[] = isReschedule
            ? ['datetime', 'barber']
            : ['datetime', 'barber', 'summary'];
              ...
              {steps.map((s, i) => (
                <div key={s} className="flex items-center gap-2 flex-1">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-heading font-semibold transition-colors ${
                    ...
                    (currentIdx > i) ? 'bg-success text-success-foreground' :
                    'bg-secondary text-muted-foreground'
                  }`}>
                    {(currentIdx > i) ? <Check size={14} /> : i + 1}
                  </div>
                  {i < steps.length - 1 && <div className={`flex-1 h-0.5 rounded-full ${
                    (currentIdx > i) ? 'bg-success' : 'bg-border'
                  }`} />}
                </div>
              ))}
            </div>
      ...
      <PageFloatingFooter>
        ...
        >
          <div className="max-w-lg mx-auto">
            ...
            {step === 'barber' && isReschedule && (
              <button
                disabled={!canConfirmReschedule || paying}
                onClick={handleConfirmBooking}
                className={`w-full py-3.5 rounded-2xl font-heading font-semibold text-sm transition-all duration-200 ${
                  canConfirmReschedule && !paying
                    ? 'bg-primary text-primary-foreground active:scale-[0.98]'
                    : 'bg-muted text-muted-foreground cursor-not-allowed'
                }`}
              >
                {paying ? 'Rescheduling…' : 'Confirm Reschedule'}
              </button>
            )}
            {step === 'summary' && (
              <div className="flex gap-3">
                <button
                  onClick={handleConfirmBooking}
                  disabled={paying}
                  className="flex-1 py-3.5 rounded-2xl font-heading font-semibold text-sm bg-primary text-primary-foreground active:scale-[0.98] transition-transform disabled:opacity-60"
                >
                  {paying ? 'Processing…' : `Pay & Confirm • ₹${cartTotal + Math.round(cartTotal * 0.18)}`}
                </button>
                <button
                  onClick={handleConfirmBooking}
                  disabled={paying}
                  className="py-3.5 px-4 rounded-2xl font-heading font-medium text-sm border-2 border-primary text-primary active:scale-[0.98] transition-transform disabled:opacity-60"
                >
                  Pay at Salon
                </button>
              </div>
