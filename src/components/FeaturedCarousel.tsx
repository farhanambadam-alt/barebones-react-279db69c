...
const FeaturedCarousel = memo(({ salons }: FeaturedCarouselProps) => {
  ...
  // Track scroll to update active index — throttled via rAF
      ...
      >
        {INFINITE_DATA.map((salon, index) => {
          ...
          return (
            <div
              key={`${salon.id}-${index}`}
              className={`shrink-0 w-[82%] md:w-[46%] lg:w-[34%] max-w-[520px] snap-center rounded-[20px] min-h-[150px] md:min-h-[170px] cursor-pointer relative overflow-hidden bg-gradient-to-r ${palette} ${
                isActive
              ...
              style={{ transform: 'translateZ(0)', willChange: 'transform, opacity', transition: 'transform 0.5s ease-out, opacity 0.5s ease-out' }}
            ...
            >
              <div className="relative z-20 w-[58%] p-4 md:p-5 flex flex-col justify-between h-full min-h-[150px] md:min-h-[170px]">
                <span className="self-start bg-accent text-accent-foreground text-[10px] md:text-[11px] font-heading font-semibold px-2.5 py-0.5 rounded-lg mb-2">
                  <DistanceLabel distance={getDistance(salon)} />
                </span>
