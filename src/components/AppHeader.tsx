      ...
      {showNotification && (
        <button
          onClick={() => {
            ...
            const needsBlur = !!active && (active.tagName === 'INPUT' || active.tagName === 'TEXTAREA');
            if (needsBlur) {
              // Defer blur so the keyboard-close animation runs on a separate
              // frame from the drawer-open transition (prevents WebView stutter).
              requestAnimationFrame(() => active!.blur());
              // Open drawer on the next frame so React commit doesn't collide
              // with the native keyboard dismissal.
